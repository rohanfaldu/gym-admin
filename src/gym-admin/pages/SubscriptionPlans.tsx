...
const SubscriptionPlans: React.FC = () => {
  ...
  const fetchSubscriptions = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/gym/${user.gymId}/subscriptions`, {
        headers: {
  ...
  const handleEditPlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const user = JSON.parse(userData);
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/gym/${user.gymId}/subscriptions/${editingPlan.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeletePlan = async (id: string) => {
    if (confirm('Are you sure you want to delete this plan?')) {
      try {
        ...
        const user = JSON.parse(userData);
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/gym/${user.gymId}/subscriptions/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
