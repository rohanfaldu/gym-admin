...
const MemberManagement: React.FC = () => {
  ...
  const fetchMembers = async () => {
    try {
      ...
      const response = await fetch(`https://gym-api-qzjz.onrender.com/api/gym/${user.gymId}/members`, {
        headers: {
  ...
  const handleDeleteMember = async (id: string) => {
    if (confirm('Are you sure you want to delete this member?')) {
      try {
        ...
        const user = JSON.parse(userData);
        const response = await fetch(`https://gym-api-qzjz.onrender.com/api/gym/${user.gymId}/members/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
